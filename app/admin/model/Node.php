...
class Node extends Model
{
    ...
    {
        $result = $this->field('id,title,pid')->order('id desc')->select();
        $str = "";
        $role = new UserType();
    ...
    /**
     * 获取有效的节点权限
     * 如果首级节点被关闭，哪么下面的子级节点也不应该显示
     */
    public function getNodeAccess($id){
        $result = $this->field('id,title,pid')->order('sort asc')->select();
        $role = new UserType();
        $rule = $role->getRuleById($id);
        ...
        $str = '';
        return self::getValidityNode($result, $rule, $str, 0);
    }
     ...
     * 采用无限循环查找
     */
    static public function getValidityNode($cate, $user_access, &$str, $pid=0)
    {
        foreach ($cate as $v){
            if($v['pid']==$pid){
                $str .= '{ "id": "' . $v['id'] . '", "pId":"' . $v['pid'] . '", "name":"' . $v['title'].'"';
                if(!empty($user_access) && in_array($v['id'], $user_access)) {
                    $str .= ' ,"checked":1';
                }
                $str .= '},';
                self::getValidityNode($cate, $user_access, $str, $v['id']);
            }
        }
        return "[" . substr($str, 0, -1) . "]";
    }
